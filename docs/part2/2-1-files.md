...
① 먼저 "● 확인 끝. 분류 계획:" 출력 후, **작업자별로 그룹 묶어** 각 단계 폴더에 들어갈 파일을 나열:
   ...
   4_PM에이전트/RFM/LOM/ → (final 파일들)

② 파일 이동 실행 (Bash/Move).

③ 실행 후 "● 정리 완료. N개 파일 모두 이동했고 raw_files/는 비었음." 출력.
   그 다음 ASCII box-drawing 표(┌ ─ ┐ ├ ─ ┤ └ ─ ┘)로 폴더별 파일 목록. **헤더는 "폴더" / "파일"**, 작업자별 순서 (S → B → LOM):

   ┌──────────────────────┬──────────────────────────────────┐
   │         폴더         │              파일                │
   ├──────────────────────┼──────────────────────────────────┤
   │ RFM/RFM(S)/raw_data/ │ (S) NX-Interferom_input.stp, ... │
   ├──────────────────────┼──────────────────────────────────┤
   │ ...                  │ ...                              │
   └──────────────────────┴──────────────────────────────────┘

④ 마지막 "판단 메모:" 섹션에 비표준 파일별로 분류 근거 bullet (파일명 명시):
   - (S) NX-Interferom_v1.stp — .stp 확장자 우선 적용해서 raw_data로 (버전 표시 규칙은 .pdf 한정)
   - (B) Manual_update_v3.pdf — _v3 버전 + .pdf 조합이라 working으로
   - RFM(B)/feedback — 해당 파일 없어 빈 폴더로 생성 (구조 일관성 유지)
```
