...
이 시간 30분 동안 우리는 **흩어진 파일들을 폴더에 정리하는 일**을 클로드 코드에게 맡겨볼 거예요. 평소에 손으로 하면 30분~1시간 걸릴 일이 명령어 한 줄로 끝나는 걸 직접 보실 수 있어요.

> 오늘 만들 두 비서를 기억하시나요?
> - **PM 비서** (4번) — 진행 상황을 보고하는 비서
> - **매뉴얼 작가 비서** (5번) — 회사 톤의 매뉴얼을 자동으로 써주는 비서
>
> 본격 비서 만들기 전에 **"AI가 내 컴퓨터에서 실제로 일한다"는 감각을 익히는 워밍업**이에요.
...
> 다음은 4 — **PM 에이전트 만들기** (90분).
> 방금과 같은 구조의 미리 준비된 RFM 폴더를 PM 비서가 들여다보고, "지금 어느 작업자가 어느 단계에 있는지" 자동으로 보고하게 만들어볼 거예요.
