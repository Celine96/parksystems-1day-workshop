...
- **2.2 PM 에이전트** = 회사 폴더를 들여다보고 **읽는** 에이전트
- **2.3 매뉴얼 작가 에이전트** = 회사 형식을 학습해서 **쓰는** 에이전트

> 💡 두 에이전트 모두 같은 `.claude/agents/` 폴더에 살아요. 즉 **PM 에이전트 만들 때 닦아둔 길(하네스)을 그대로 활용**해서 두 번째 에이전트를 빠르게 만들 수 있어요. 이게 하네스의 힘이에요.

> 🎯 **오늘 톤 알림**: 풀 매뉴얼 한 권을 자동 생성하지 않아요. **"이런 방향이 가능하다"** 를 Introduction 한 페이지 변환으로 체험하는 게 목표예요.

---

## [실습] 매뉴얼 작가 에이전트 만들기 (40분)
