# 5. 프로젝트 3 : 사내 문체 매뉴얼 자동 생성 에이전트

> 오늘 만들 두 번째 에이전트, **에디터 에이전트**의 시간이에요. 전체 메뉴얼중에 **전체 메뉴얼중에 Introduction 한 페이지** 변환으로 시작합니다.
...
처음부터 백지에서 쓰는 게 아니라, **회사 형식을 알고 있는 에디터 에이전트**가 raw data를 받아서 같은 톤으로 옮겨주는 거예요.
...
에이전트가 알아야 할 것 (9개 섹션):
1. 톤 (한국어 격식체 "~합니다", 영문 기술 용어 원어 유지, 능동·현재시제, 약어 첫 등장 시 풀네임 병기)
2. 챕터 시작 페이지 구조 (좌측 사이드 헤더, 우측 본문 컬럼, Figure 캡션 규칙, 홀수 페이지 시작)
3. Preface 5단 구성 (제품 소개 → 매뉴얼 목적·표준 → 챕터 안내 → 모델 면책 → 영업·CS 연락)
4. Spec 표 양식 (Table N.N 캡션 형식, 항목·단위·범위·각주 규칙)
5. 푸터 형식 (NXT1518-IN2-C0.00-KO + 발행일 패턴, 짝수/홀수 페이지 구분, PPT 별도)
6. 백 매터 (Park Systems Resources and Services 정형 단락 보존)
7. 매뉴얼별 섹션 구성 차이 (IN/MT/OP 각 특징)
8. 안전 신호어 5단계 (NOTE → NOTICE → CAUTION → WARNING → DANGER)
9. raw data → 매뉴얼 변환 작업 순서 (매뉴얼 종류 파악 → 레퍼런스 선택 → 작성 → 5_매뉴얼/output/ 저장 → 7항목 자가 점검)
```
...
<SectionTitle icon="🔍" title="결과 파일 열어 점검" sub="markdown 일부만으론 부족하니 본인 파일로 확인" spacious />

**1) 본인 파일 열기**

> 💡 파일 탐색기에서 `5_매뉴얼/output/Chapter1_Intro.md` 더블클릭, 또는 클로드 코드에 `"방금 만든 5_매뉴얼/output/Chapter1_Intro.md 파일을 열어서 보여줘"`라고 부탁.

**2) ✅ 확인 포인트** — 결과물이 회사 톤과 맞는지
...
두 에이전트 정의(`.md` 파일)는 프로젝트 로컬 `.claude/agents/` 디렉터리에 저장돼요. 클로드 코드는 프로젝트 루트(`CLAUDE.md`가 있는 폴더)에서 세션을 시작할 때 이 디렉터리를 자동 스캔해 **호출 가능한 에이전트로 등록**합니다. 한 번 만들어두면 같은 프로젝트 안에서는 언제든 재사용 가능해요.

> 💡 **포인트**: 에이전트 정의는 `.md` 파일이라 폴더와 함께 그대로 전달돼요. 즉 **에이전트를 한 번 만드는 노력 = 팀 전체가 두고두고 쓰는 자산**이에요.

## [응용 - 본 수업 끝나고] 내 업무에 적용하려면?
